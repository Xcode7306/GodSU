...
两个配套的 Claude Code / Claude Skill：**把经历写强**，再**把简历排好看**。方法论逆向自一份爆火的校招简历（多模态算法工程师方向）——手法、句式、视觉规格都是逐段拆解出来的。

| Skill | 管什么 | 输出 |
|---|---|---|
| [`resume-crafting`](resume-crafting/) | 文案。挖料 → 按 11 个手法 / 13 个句式改写 → 自查 | 高信号的简历正文 |
| [`resume-typeset`](resume-typeset/) | 排版。填模板 → 无头 Chrome 渲染 → 压成一页 | 一页 A4 PDF |
...
```bash
git clone https://github.com/youngbeauty/GodSU.git
cp -R GodSU/resume-crafting GodSU/resume-typeset ~/.claude/skills/
```

放 `~/.claude/skills/` 是全局可用；只想在某个项目里用就放该项目的 `.claude/skills/`。

之后直接说「帮我优化简历」「把这份简历排成 PDF」，或 `/resume-crafting`、`/resume-typeset`。
...
依赖：macOS + Google Chrome；验收步骤需要 `pip3 install pymupdf`。
